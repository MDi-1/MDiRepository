...
class BoardTestSuite {
    ...
    @DisplayName("Exercise 7.6 test to get average out of progress task list")
    @Test
    void testAddTaskListAverageWorkingOnTask() {
        ...
        // When
        List<TaskList> tasx = new ArrayList<>();
        tasx.add(new TaskList("In progress"));
        double average = project.getTaskLists().stream()
             .filter(tasx::contains)
             .flatMap(tl -> tl.getTasks().stream())
             .map(d -> Math.abs(DAYS.between(d.getDeadline(), d.getCreated())))
             .mapToLong(Long::longValue)
             .average().getAsDouble();
        System.out.println("The average days calculated from created date to deadline: " + average);

        // Then
        assertTrue(18.3 < average && average < 18.4);
    }
}